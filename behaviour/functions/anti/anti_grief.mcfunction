...
#detect
execute as @a[scores={grief_on=0}] run execute as @e[type=tnt,tag=!admin] run tellraw @a {"rawtext":[{"text":"§6[§eAnti Cheats§6]§r§c§l "},{"selector":"@p[tag=!admin]"},{"text":" §r§4Was detected summoning a §l§cTNT!§r"}]}
...
execute as @a[scores={grief_on=0}] run execute as @a[tag=!admin] run fill ~8 ~5 ~8 ~-8 ~-5 ~-8 air replace tnt
execute as @a[scores={grief_on=0}] run execute as @a[tag=!admin] run fill ~8 ~5 ~8 ~-8 ~-5 ~-8 air replace fire